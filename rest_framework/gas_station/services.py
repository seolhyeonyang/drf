...
class Service():
    ...
    def get_url(self):
        ...
        gu_name = [option.get_getattribute__('value') for option in gu_list]
        gu_name.remove('')
        print(gu_name)
    ...
    def gas_station_price_info(self):
        ...
        # SK네트웍스(주) 효진주유소 1 서울특별시 성동구 동일로 129(성수동2가) 1654 N SK에너지 서울특별시

        stations.loc[stations[stations['구'] == '서울특별시']] = '성동구'
        stations['구'].unique()


        # print(stations[stations['구'] == '특별시'])
        # 서현주유소 서울 특별시 도봉구 방학로 142(방학동) 1524 Y S - OIL 특별시
        stations.loc[stations[stations['구'] == '특별시']] = '도봉구'
        stations['구'].unique()
